...
export default function BookingFlow({ courts }: BookingFlowProps) {
  ...
  const handleBooking = async () => {
    ...
    if (result.success) {
      ...
      // Recargar los turnos para que desaparezca el que acabamos de reservar
      const updatedSlotsResponse = await getAvailableSlots(selectedCourt, selectedDate);
      if (updatedSlotsResponse.success && updatedSlotsResponse.data) {
        setAvailableSlots(updatedSlotsResponse.data as any);
      } else {
        setAvailableSlots([]);
      }
